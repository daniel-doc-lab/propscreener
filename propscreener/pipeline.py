...
from .sources.cvr import ApiCvrMcp, CvrApi, CvrElastic, StatstidendeCvr, enrich_with_cvr
...
class Pipeline:
    ...
    def enrich_cvr(self, case: BankruptcyCase) -> None:
        enrich_with_cvr(case, self.cvrapi, self.cvr_es, self.cvr_fallback, self.cvr_mcp)
    ...
    def active_sources(self) -> list[str]:
        src = [f"statstidende:{self.s.statstidende_mode}", "cvrapi", "apicvr-mcp", "regnskab-xbrl", "dawa"]
        if self.cvr_es:
            src.append("cvr-es")
        if self.ejf:
