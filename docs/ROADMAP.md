...
Implementeret fra forslagslisten nedenfor: **#2 watchlist, #3 kort, #5 kurator-profil, #6 koncern-visning
(afledt), #7 tvangsauktionsregister, #8 faktaark, #10 mikro-grafer, #11 frist-kalender.**
Datafordeler: EBR-indeks (adresser) bygges ugentligt med API-nøglen; EJF/VUR afventer OAuth-secret
og godkendt anmodning.
...
* Ejerfortegnelsen: fildownload-klient og indeksbygning er klar; EJF-listen er tom indtil anmodningen er godkendt (OAuth).
* Tvangsauktioner matches på "Skødehaver ifølge tingbogsattest" og udstilles desuden som selvstændigt register.
...
| 2.2 | ✅ Watchlist og noter pr. bo (gemmes i browser); næste: delt liste via artifact-database | Arbejdsflow: "set", "kontaktet", "bud afgivet" | – |
| 2.3 | ✅ Kort over boer og ejendomme (inlinet SVG); næste: klynger og kortfliser | Geografisk screening; se porteføljer i ét blik | 1.1 |
| 2.4 | Reelle ejere, ledelse og koncernstruktur via CVR system-til-system (afledte relationer ✅) | Ejendomme i datterselskaber; gengangere blandt ejere | Aftale hos Erhvervsstyrelsen |
...
| 3.3 | Eksport til Excel med pivot-klar struktur (PDF-faktaark ✅) | Deling internt og med banker/partnere |
...
6. ✅ (afledt) **Koncern-visning.** Når moderselskabet går konkurs, slå datterselskaber op og vis deres
   ejendomme samlet. *Implementeret som afledte relationer* (samme adresse, ejer/ledelse, navnestamme,
   kurator + dekretdato) på tværs af alle dekreter i perioden, fordi ingen åben CVR-kilde giver
   ejerstruktur (datacvr.virk.dk er bag Cloudflare, cvrapi.dk har kvote, Erhvervsstyrelsens
   system-til-system kræver aftale). Registrerede koncernforhold kræver stadig CVR-aftalen.
